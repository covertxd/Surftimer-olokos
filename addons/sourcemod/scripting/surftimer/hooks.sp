...
public Action Event_OnFire( Handle event, const char[] name, bool dontBroadcast )
{
	...
	if (client && IsClientInGame( client ) && g_hAttackSpamProtection.BoolValue )
	{
		char weapon[64];

		GetEventString( event, "weapon", weapon, sizeof( weapon ) );

		if ( StrContains( weapon, "knife" ) == -1 && g_AttackCounter[client] < 41 )
		{
			g_AttackCounter[client]++;

			if ( StrContains( weapon, "grenade" ) != -1 || StrContains( weapon, "flash" ) != -1 )
			{
...
public Action Event_OnPlayerSpawn( Handle event, const char[] name, bool dontBroadcast )
{
	...
	SetEntData( client, FindSendPropInfo( "CBaseEntity", "m_CollisionGroup" ), ( IsFakeClient || g_hCvarNoBlock.BoolValue ) ? 2 : 5, 4, true );
	...
	if ( client )
	{
		...
		bool isClientValid = IsValidClient( client );
		bool isClientOnTeam = GetClientTeam( client ) > 1;
		...
		switch ( g_iCurrentStyle[client] )
		{
			...
			case 6: // 6 fastforward
			{
				SetEntPropFloat( client, Prop_Data, "m_flLaggedMovementValue", 1.5 );
			}
			default: // 0 normal, 1 hsw, 2 sw, 3 bw
			{
		...
		// Strip Weapons
		if ( isClientValid && isClientOnTeam )
...
public Action Say_Hook( int client, const char[] command, int argc )
{
	...
	if ( isClientValid && g_ClientRenamingZone[client] )
	{
	...
	if ( isClientValid )
	{
		...
		//{
		//	if ( StrEqual( g_BlockedChatText[i], sText ) )
		//		return Plugin_Handled;
		//}
		...
		if ( g_iWaitingForResponse[client] > -1 )
		{
			...
			switch ( g_iWaitingForResponse[client] )
			{
				...
				case 5: // Zone set clients Target Name
				{
		...
		// !s & !stage Commands
		if ( StrEqual( sText, "!s", false ) == 0 || StrEqual( sText, "!stage", false ) == 0 )
			return Plugin_Handled;

		// !b & !bonus Commands
		if ( StrEqual( sText, "!b", false ) == 0 || StrEqual( sText, "!bonus", false ) == 0 )
			return Plugin_Handled;

		if ( checkSpam( client ) )
			return Plugin_Handled;

		parseColorsFromString( sText, sizeof( sText ) );

		// Hide ! commands
		if ( StrEqual( sText, "!" ) )
			return Plugin_Handled;

		// lowercase if has at least 2 characters
		if ( ( sText[0] == '/' || sText[0] == '!' ) && sText[1] )
		{
			// IsCharUpper included in CharToLower
			// https://github.com/alliedmodders/sourcemod/blob/1.10-dev/plugins/include/string.inc#L420
		...
		if ( ( IsChatTrigger() && sText[0] == '/' ) || ( sText[0] == '@' && CheckCommandAccess( client, "", ADMFLAG_GENERIC, true ) ) )
			return Plugin_Continue;
		...
		if ( g_hPointSystem.BoolValue && g_hColoredNames.BoolValue && g_bDbCustomTitleInUse[client] )
		{
			setNameColor( szName, g_iCustomColours[client][0], sizeof( szName ) );
		...
		{
			if ( g_hColoredNames.BoolValue && !g_bDbCustomTitleInUse[client] )
				Format( szName, sizeof( szName ), "{%s}%s", szChatRankColor, szName );

			if ( g_hCountry.BoolValue )
			{
				CPrintToChatAll( "%t", IsPlayerAlive( client ) ? "Hooks6" : "Hooks7", g_szCountryCode[client], szChatRank, szName, sText );

				return Plugin_Handled;
			}

			CPrintToChatAll( "%t", IsPlayerAlive( client ) ? "Hooks8" : "Hooks9", szChatRank, szName, sText );
...
// TODO: Refactor references to use colorlib and remove method
...
{
	if ( IsValidClient( other ) && g_hTriggerPushFixEnable.BoolValue == true )
	{
...
public Action OnEndTouchPushTrigger( int entity, int other )
{
	if ( IsValidClient( other ) && g_hTriggerPushFixEnable.BoolValue == true )
	{
		if ( !IsFakeClient( other ) && IsValidEntity( entity ) )
...
{
	if ( !IsFakeClient( other ) && IsValidClient( other ) )
	{
...
public Action Event_OnPlayerHurt( Handle event, const char[] name, bool dontBroadcast )
{
	if ( !g_hCvarGodMode.BoolValue &&  g_hAutohealing_Hp.IntValue > 0 )
	{
		...
		{
			int total = remainingHeatlh + g_hAutohealing_Hp.IntValue;

			SetEntData(client, FindSendPropInfo("CBasePlayer", "m_iHealth"), total > 100 ? 100 : total );
		}
	}
...
public Action OnPlayerRunCmd( int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2] )
{
	...
	else if ( g_iCurrentStyle[client] == 5 ) // Slow Motion
	{
		...
		if ( TR_DidHit() )
		{
			...
			// Check if client is on a surf ramp
			// if client is stuck put him back on the ramp
	...
	if ( IsPlayerAlive( client ) )
	{
		...
		g_bOnGround[client] =  GetEntityFlags( client ) & FL_ONGROUND ? true : false;

		float newVelocity[3];

		// Slope Boost Fix by Mev, & Blacky
		// https://forums.alliedmods.net/showthread.php?t=266888
		if ( g_hSlopeFixEnable.BoolValue == true && !IsFakeClient( client ) )
		{
			...
			if ( g_bOnGround[client] == true && g_bLastOnGround[client] == false )
			{
				...
				float vPos[3], vMins[3], vMaxs[3], vEndPos[3];
		...
		float velocity = ( newVelocity[0] == 0.0 && newVelocity[1] == 0.0 && newVelocity[2] == 0.0 ) ? vel : newVelocity;

		RecordReplay( client, buttons, subtype, seed, impulse, weapon, angles, velocity );

		if ( IsFakeClient( client ) )
			PlayReplay( client, buttons, subtype, seed, impulse, weapon, angles, velocity );
		...
		if ( ( g_bTimerRunning[client] || g_bWrcpTimeractivated[client] ) && iGroundEntity == -1 && ( GetEntityFlags( client ) & FL_INWATER ) == 0 && fAngle != 0.0 )
		{
			...
			if ( SquareRoot( Pow( fAbsVelocity[0], 2.0 ) + Pow( fAbsVelocity[1], 2.0 ) ) > 0.0 )
			{
				...
				else
				{
					if( fAngle != 0 )
					{
						int buttonDirection = fAngle > 0 ? IN_MOVELEFT : IN_MOVERIGHT;

						g_iTotalMeasures[client]++;

						if ( buttons & buttonDirection || ( buttons & IN_FORWARD && buttons & buttonDirection ) )
							g_iGoodGains[client]++;
					}
				}
			}
...
public Action Event_PlayerJump( Handle event, char[] name, bool dontBroadcast )
{
	...
	if ( !client && !IsPlayerAlive( client ) && !IsClientObserver( client ) )
		return Plugin_Continue;
	...
	if ( IsValidClient( client ) && !IsFakeClient( client ) )
	{
		...
		if ( g_hLimitSpeedType.IntValue == 1 )
		{
			...
			float timeDifference = time - g_iLastJump[client];

			g_iLastJump[client] = time;
		...
		if ( g_hOneJumpLimit.BoolValue && g_hLimitSpeedType.IntValue == 1 )
		{
			if ( g_bInStartZone[client] || g_bInStageZone[client] )
			{
				if ( g_mapZones[zoneid].OneJumpLimit == 1 )
				{
					if ( !g_bJumpedInZone[client] )
					{
						...
						g_fJumpedInZoneTime[client] = time;
					...
					else
					{
						...
						float timeDifference = time - g_fJumpedInZoneTime[client];


						if ( timeDifference <= 0.9 && !( g_iCurrentStyle[client] == 7 ) )
						{
...
public Action DelayedVelocityCap( Handle timer, DataPack pack )
{
	...
	for (int i = 0; i < sizeof( CurVelVec ); i++ )
	{
		if( CurVelVec[i] == 0.0 )
			CurVelVec[i] = 1.0;
	}
